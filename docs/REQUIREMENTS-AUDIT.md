...
38. **★★★ 正则写错了却"成功"了 —— 静默未命中比崩溃更危险**

    写「检测 DSH 更新 → 更新 README 版本要求」时，我用了这条正则：
    ...
    **完全没有「DSH 版本」这四个字**，所以这条正则**一次都不会命中**。

    后果的严重性在于它**不报错**：脚本照常输出
    「✓ 已更新文档」，人看到这行就放心了 ——
    而 README 其实**一个字都没改**。

    **如果我没有回头读 diff，这个 bug 会一直躺在那儿**，
    每次"自动更新版本要求"都是空转。

    **修法**：`subn` 返回的替换次数为 0 时**显式报警**：

    ```python
    if n:
        changes.append(f"README 的 DSH 版本要求 → {dsh_version}")
    else:
        changes.append("⚠️ 没找到 DSH 版本要求那一行（正则未命中）")
    ```

    并加了断言把它钉住（`check-dsh-update.js` 的【2】组）。

    **教训**：**"没做"必须和"做成了"一样响亮。**
    任何"查找并替换"的代码，都要把"没找到"当成**错误**报出来，
    而不是当作"无需修改"。静默未命中是自动化脚本最典型的烂法 ——
    它伪装成成功。

39. **★★ 沙箱不允许进程管道 —— 但换个 stdio 就能跑**

    给 Python 脚本写 JS 回归测试时，`spawnSync` 直接失败：
    ...
    这是 DSH 沙箱的已知边界（不允许创建命名管道）。
    我第一反应是"那这个套件写不了"，于是让它**跳过** ——
    但那样等于没有测试，而且"跳过"很容易变成永久的借口。
    ...
    **`stdio: 'ignore'` 能正常执行，只是拿不到 stdout。**
    既然拿不到输出，就**让 Python 把结果写进临时文件**，Node 再读回来。

    这样既绕开了管道限制，又保留了完整的断言能力
    （最终 36 项断言全部真跑，没有跳过）。

    **教训**：**遇到环境限制，先穷举替代通道，再考虑降级。**
    "跳过测试"应当是**最后**的选项，而不是第一反应。

40. **★★★ 契约断言写在「形参拼写」上 —— 升级一改名就假红**

    新版本适配链第一次真跑起来（用 0.1.7-rc.2 替 0.1.5-rc.1），
    两条断言立刻红了：

    ```
    [FAIL] ★ uiWorkspace.openSession(sessionId) 真实存在
    [FAIL] ★ uiWorkspace.archiveSession(sessionId) 真实存在（async）
    ```
    ...
    |---|---|---|
    | `openSession` | `openSession(sessionId)` | `openSession(target)` |
    | `archiveSession` | `async archiveSession(sessionId)` | `async archiveSession(sessionId, options = {})` |

    我第一反应是"**插件被 DSH 新版打破了**" —— 这正是适配链要抓的东西。
    但读了两版**实现体**之后发现完全相反：

    ```js
    // 旧：openSession(sessionId) { this.sessions.open(sessionId); }
    // 新：openSession(target) {
    //       this.replaceMain(target, ...);  // 内部 typeof target === "string" ? ... : target
    //     }
    ```

    **两版都接受裸 sessionId 字符串**。新版只是**多支持**了对象形式
    （所以把 `sessionId` 改名成更准确的 `target`）；`archiveSession` 新增的
    `options` 也**有默认值**，不传就是旧行为。

    **结论：插件没坏，是我的测试坏了。**

    ### 为什么这个错比"漏测"更隐蔽

    断言写的是 `/openSession\(sessionId\)\s*\{/` —— 它锁的不是
    "能不能传字符串进去"，而是"**形参**是不是恰好叫 `sessionId`"。
    形参名是**实现细节**，DSH 想改就改，改了不影响任何调用方。

    于是这条断言会在**插件完全正常**时报红，把注意力引向错误的方向。
    假红和漏测一样有害：**它消耗的是对测试的信任。**
    ...
    ```js
    check('openSession 实现里显式处理 typeof === "string"',
          /typeof\s+target\s*===\s*"string"/.test(wsText))   // ✗ 又错了
    ```

    **旧版立刻红了** —— 因为旧版走的是完全不同的内部路径
    （`this.sessions.open(...)`，压根没有 `replaceMain`）。
    我**换了个实现细节来锁**，等于没改。

    最终定稿只核对**可观测的调用契约**：

    ```js
    // 首个形参不是对象解构 → 传裸字符串仍然可用
    !/^\s*\{/.test(openSig[1])
    // 新增的参数必须有默认值 → 不传就是旧行为
    archSig[1].split(',').length === 1 || /=\s*/.test(后面的参数)
    ```

    改完**两版都过**（各 33/33），这才是契约断言该有的样子。

    **教训（本条最值钱的一句）**：
    **断言要锁「调用方的体验」，不要锁「被调用方的写法」。**
    判据很简单 —— 问自己："这个测试失败，能不能推出**用户会看到问题**？"
    推不出，就是过拟合。

    ### 顺带：这条把适配链的价值证明了

    适配链**第一次真跑**就逼我读了两版实现体，从而确认
    `0.1.7-rc.2` 上插件可用。如果我没跑它，只会有两种结局：
    要么盲目认为"应该没问题"，要么被假红误导去改**本来正确**的代码。

41. **★★★ 版本号永不前进 —— 第二次发版会静默改写上一个 Release**

    用户提了一句"应该需要保留之前的 release 做版本管理"，
    我顺手核对了一下，发现一个**已经发生**的问题：
    ...
    ```python
    tag = next_tag(pkg["version"])   # 永远 next_tag("0.1.0") = "v0.1.1"
    ```

    **`next_tag` 从 `package.json` 推导，但没有任何代码把新版本写回去。**
    于是每次发版都算出同一个 `v0.1.1`。

    而 `make_release` 对「tag 已存在」的处理是 **PATCH 更新**：

    ```python
    if already_exists:
        PATCH /releases/{id}   # 直接改掉上一个 Release
    ```

    两者叠加的后果不是报错，而是：**第二次发版会把 v0.1.1 的内容
    悄悄换掉**。版本管理表面正常，实则**丢历史**。

    ### 为什么这个 bug 特别难发现

    它**完全静默**：发版返回 `ok: True`，URL 正常，网页上有一个 Release。
    只有去比对 `package.json` 和 tag 列表，才会发现对不上 ——
    而没人会无缘无故去比对。

    更糟的是它**只在第二次发版时才有害**。第一次永远是对的，
    所以"我试过一次，成功了"完全不能证明它没问题。

    ### 修法（三处）

    1. **推导以已有 tag 为准**，不再看 `package.json`：

       ```python
       def next_tag(current, existing=None):
           parsed = [语义化解析(t) for t in (existing or [])]
           if parsed: return max(parsed) + patch(1)   # 单调递增
           ...  # 没有 tag 才退回 package.json
       ```

    2. **把新版本写回 `package.json` 并提交** —— 补上缺失的那一步。

    3. **默认拒绝覆盖已有 Release**：

       ```python
       if already and not force_update:
           return {"ok": False, "error": "tag 已存在，**拒绝覆盖**（保护历史版本）…"}
       ```

       想要覆盖必须显式 `--force-update`。

    实测验证：用已存在的 `v0.1.1` 发版 → **被拒绝**，
    且复查 `v0.1.1` 的标题与正文**原样未动**。

    **教训**：**"成功"不等于"做对了"。**
    凡是**有状态**的自动化（发版、迁移、部署），
    都要问一句："**第二次跑会怎样？**"
    第一次成功什么都证明不了 —— 而这正是这类 bug 的藏身之处。
...
⚠️ 另需注意：41 条里有多条是**在真实 DSH 里重启后才暴露**的
